...
#[macro_export]
macro_rules! impl_vector3d {
    ($vec:ident) => {
        impl Vector3d for $vec {
            fn x(&self) -> <Self as Vector>::Float {
                self.x.clone()
            }
...
impl_min_max_for_vector!(f64);

impl_vector2d!(Point2d);

impl_vector3d!(Point3d);

impl Cross3d for Point3d {
    fn cross(&self, other: &Self) -> Self {
